...
HoloLens, azure ile tümleştirilmiş Windows bir mobil cihazdır.  En iyi, kablosuz ağ kullanılabilirliği (wi-fi) ve wi-fi erişimi olan ticari ortamlarda Microsoft hizmetleri.
...
1. Şirket kullanıcılarının şirket içinde olduğundan emin Azure Active Directory (Azure AD).
 ...
 **Kullanıcılar cihazları Azure AD'ye katabilirsiniz'i All olarak *ayarlayın***
    ...
    1. **2. Seçenek:** Seçili kullanıcılara/gruplara cihazları Azure AD'ye katma izni verme **Yönetici olarak Azure portal'da** oturum açma Azure Active Directory Cihazlar Cihazı Ayarlar Kullanıcılar cihazları Azure AD'ye katarak Azure AD'ye katılabilir ve  >    >    >    >
 Bu görüntüde Azure **** 
 ![ AD'ye Katılmış Cihazların Yapılandırmasını gösteren Seçili Görüntü](images/azure-ad-image.png)
...
1. Koşullu Erişim, mobil cihazların ve mobil uygulamaların şirket kaynaklarına erişmesine izin verir/erişimini geri verir. CA Dağıtımınızı planlama ve En İyi [](/azure/active-directory/conditional-access/best-practices) [Yöntemler olmak için yararlı bulabilirsiniz.](/azure/active-directory/conditional-access/plan-conditional-access)
...
1. Microsoft Store  
  Uygulama Microsoft Store, uygulamaları uygulama dağıtım ve uygulama dağıtımı için en iyi HoloLens.  Mağazada zaten mevcut olan HoloLens bir dizi temel uygulama vardır veya kendi uygulamalarınızı [yayımlayın.](/windows/uwp/publish/)  
...
1. Intune veya başka bir mobil cihaz yönetimi çözümü aracılığıyla uygulama dağıtımı ve yönetimi  
  Intune dahil olmak üzere çoğu mobil cihaz yönetimi çözümü, iş hattı uygulamalarını doğrudan kayıtlı bir cihaz kümesine dağıtmak için bir yol sağlar.  [Intune uygulama yüklemesi için bu makaleye bakın.](/intune/apps-deploy)
