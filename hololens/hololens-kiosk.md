...
2. **Birden çok uygulama bilgi** noktası modu Başlat menüsü bilgi noktası yapılandırmasında belirtilen uygulamaları gösterir. İsterseniz otomatik olarak başlatilecek bir uygulama seçilebilir. <br> *Örnek kullanım:* Başlat menüsünde yalnızca Mağaza uygulamasını, Geri Bildirim Merkezi ve Ayarlar gösteren bir cihaz.
...
\*Devre dışı bırakılmış özellikleri etkinleştirme veya ses komutlarının devre dışı bırakılmış özelliklerle etkileşim kurma ve Cortana hakkında daha fazla bilgi HoloLens bkz. uygulamalar için [AUMID'ler.](hololens-kiosk-reference.md#hololens-application-user-model-ids-aumids)
...
4. Bilgi noktası modunun çalışma zamanı sağlama paketleri HoloLens (MDM) sunucusu aracılığıyla Cihaz Yönetimi olup olmadığını belirler.
